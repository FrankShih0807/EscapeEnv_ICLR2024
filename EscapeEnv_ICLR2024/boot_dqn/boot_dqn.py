...
from EscapeEnv_ICLR2024.boot_dqn.estimator import BootDQNEstimator
from EscapeEnv_ICLR2024.common.torch_layers import EnsembleNet
from EscapeEnv_ICLR2024.common.buffers import BaseBuffer

class BootDQN(BaseAgent):
    def __init__(self, 
                 env,
                 estimator_class=BootDQNEstimator, 
                 buffer_class=BaseBuffer,
                 network_class=EnsembleNet, 
                 **kwargs):
        super().__init__(env, estimator_class, buffer_class, network_class, **kwargs)
    def _build_network(self):
        self.network = self.network_class(n_heads=self.estimator_kwargs['n_heads'], input_size=self.state_dim, output_size=self.num_actions, net_arch=self.net_arch, activation_fn=self.activation_fn)
