...
from EscapeEnv_ICLR2024.qrdqn.estimator import QRDQNEstimator
from EscapeEnv_ICLR2024.common.torch_layers import QuantileNetwork
from EscapeEnv_ICLR2024.common.buffers import BaseBuffer

class QRDQN(BaseAgent):
    def __init__(self, 
                 env,
                 estimator_class=QRDQNEstimator, 
                 buffer_class=BaseBuffer,
                 network_class=QuantileNetwork, 
                 **kwargs):
        super().__init__(env, estimator_class, buffer_class, network_class, **kwargs)
    def _build_network(self):
        self.network = self.network_class(input_size=self.state_dim, num_actions=self.num_actions, n_quantiles=self.estimator_kwargs['n_quantiles'], net_arch=self.net_arch, activation_fn=self.activation_fn)
